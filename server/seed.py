...
def seed_crafts():
    for craft_info in CRAFT_DATA:
        new_craft = Craft(
            image = craft_info['image'],
            title = craft_info['title'],
...
def seed_materials():
    all_materials = set(material for craft_info in CRAFT_DATA for material in craft_info['materials'])
...
def associate_crafts_materials():
    for craft_info in CRAFT_DATA:
        new_craft = db.session.query(Craft).filter_by(title=craft_info['title']).first()
        for material_name in craft_info['materials']:
            material_instance = db.session.query(Materials).filter_by(name=material_name).first()
            new_craft_material = CraftMaterials(craft_id=new_craft.id, material_id=material_instance.id)
            db.session.add(new_craft_material)
